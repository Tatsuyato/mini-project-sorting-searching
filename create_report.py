...
for x in ['เพิ่มข้อมูลนักเรียน ตรวจสอบรหัสซ้ำ และตรวจสอบคะแนนให้อยู่ในช่วง 0–100','แสดงรายชื่อนักเรียนพร้อมคะแนนและเกรด','ลบข้อมูลนักเรียนและโหลดข้อมูลตัวอย่าง 12 รายการ','โหมดข้อมูลสังเคราะห์จำลองสำหรับการสาธิต (-data-test / --data-test / --mock-data) โหลดข้อมูลนักเรียนตามจำนวนที่กำหนด ค่าเริ่มต้น 12 รายการ สูงสุด 1,000,000 รายการ','เรียงข้อมูลตามคะแนน รหัสนักเรียน หรือชื่อ ทั้งแบบน้อยไปมากและมากไปน้อย','ค้นหาด้วยรหัสหรือคะแนนด้วย Sequential Search และ Binary Search','แสดงขั้นตอน Trace แบบจำกัดจำนวนบรรทัดเพื่อให้เหมาะกับการสาธิต','แสดง Benchmark ของ Sorting ทั้ง 4 วิธี โดยไม่ทำให้โหมด Trace กระทบผล Benchmark']:
    story.append(B(x))
story += [P('3. โครงสร้างโปรเจกต์', 'THH1')]
story.append(P('<font name="Loma">mini-project-sorting-searching/<br/>├── algorithms/ — sorting.py, searching.py<br/>├── models/ — student.py<br/>├── services/ — mock_data.py, score_manager.py<br/>├── tests/ — test_sorting.py, test_searching.py, test_score_manager.py, test_mock_data.py<br/>├── main.py — เมนูโปรแกรมหลัก (รองรับ -data-test, --data-test, --mock-data, --mock)<br/>└── README.md — คู่มือ ตารางความซับซ้อน และข้อชี้แจงจริยธรรมข้อมูล</font>', 'THCode'))
story += [P('การออกแบบแบ่งเป็นชั้น ได้แก่ โมเดลข้อมูล (Student) แกนอัลกอริทึม ฟังก์ชันบริการจัดการข้อมูลและข้อมูลจำลอง และส่วนติดต่อผู้ใช้แบบ Command Line ทำให้สามารถทดสอบและอธิบายแต่ละส่วนแยกกันได้', 'THBody'), PageBreak()]
...
story += [P('5.1 Sequential Search', 'THH2'), P('เริ่มจากตำแหน่งแรก เปรียบเทียบค่าที่ค้นหากับสมาชิกปัจจุบัน หากตรงกันให้บันทึกผล หากไม่ตรงให้ขยับไปตัวถัดไป วิธีนี้ใช้ได้กับข้อมูลที่ยังไม่เรียง แต่ถ้าข้อมูลอยู่ท้ายรายการหรือไม่พบ จะต้องตรวจหลายรายการ', 'THBody'), P('5.2 Binary Search', 'THH2'), P('กำหนดขอบเขตซ้ายและขวา แล้วคำนวณตำแหน่งกลาง เปรียบเทียบค่ากลางกับค่าที่ต้องการ หากค่าค้นหามากกว่าให้เลื่อนไปครึ่งขวา หากน้อยกว่าให้เลื่อนไปครึ่งซ้าย ทำซ้ำจนพบหรือขอบเขตหมด โปรแกรมจะตรวจสอบก่อนว่าข้อมูลเรียงแล้ว หากยังไม่เรียงจะเสนอให้เรียงด้วย Merge Sort ก่อน', 'THBody'), P('<b>ข้อควรระวัง:</b> ห้ามใช้ Binary Search กับข้อมูลที่ยังไม่เรียง เพราะผลลัพธ์อาจผิด แม้โค้ดจะทำงานโดยไม่มีข้อผิดพลาดก็ตาม', 'THBody')]
story += [P('6. โหมด Trace และโหมดข้อมูลจำลองสำหรับการสาธิต', 'THH1'), P('6.1 โหมด Trace', 'THH2'), P('ในเมนูสาธิตการเรียงลำดับ โปรแกรมจะถามว่า “ต้องการแสดงขั้นตอนการทำงานจริง (Trace) หรือไม่?” หากตอบ Y โปรแกรมจะบันทึกและแสดงขั้นตอนสำคัญ เช่น คู่ข้อมูลที่กำลังเปรียบเทียบ การสลับค่า การแทรก การเลือกค่าต่ำสุด และการแบ่ง/ผสานของ Merge Sort ส่วน Search จะแสดงตำแหน่งหรือขอบเขตที่กำลังตรวจสอบ โหมด Trace ถูกแยกจาก Benchmark เพื่อให้สถิติการเปรียบเทียบยังสะท้อนการทำงานของอัลกอริทึมตามปกติ', 'THBody'), P('6.2 โหมดข้อมูลสังเคราะห์จำลองสำหรับการสาธิต (Synthetic Mock Data Mode)', 'THH2'), P('โปรแกรมรองรับการรันด้วยคำสั่ง <b>-data-test</b>, <b>--data-test</b> หรือ flag เดิม <b>--mock-data</b> / <b>--mock</b> เพื่อโหลดข้อมูลสังเคราะห์ขนาด 35 รายการ (รหัส TEST0001–TEST0035) แทนชุดข้อมูลตัวอย่างเดิม ทำให้เห็นความแตกต่างของจำนวน Comparisons และ Swaps ระหว่าง O(n²) เช่น Bubble Sort (~592 ครั้ง) กับ O(n log n) เช่น Merge Sort (~147 ครั้ง) ได้อย่างชัดเจนในการสาธิต', 'THBody'), PageBreak()]

story += [P('7. วิธีติดตั้งและใช้งาน', 'THH1'), P('โปรเจกต์ใช้ Python Standard Library จึงไม่ต้องติดตั้งไลบรารีภายนอก แนะนำ Python 3.9 ขึ้นไป', 'THBody'), P('คำสั่งเปิดโปรแกรม', 'THH2'), P('cd /home/ryu/mini-project-sorting-searching<br/># โหมดปกติ (ข้อมูลตัวอย่าง 12 รายการ)<br/>python3 main.py<br/># โหมดข้อมูลสังเคราะห์จำลองสำหรับการสาธิต (Mock Data 35 รายการ)<br/>python3 main.py -data-test  # หรือ --data-test, --mock-data, --mock', 'THCode'), P('เมนูหลัก', 'THH2')]
menu=[['เมนู','การทำงาน'],['1','แสดงรายชื่อนักเรียนทั้งหมด'],['2','เพิ่มข้อมูลนักเรียนและคำนวณเกรด'],['3','ลบข้อมูลด้วยรหัสนักเรียน'],['4','โหลดชุดข้อมูลตัวอย่าง / โหลดข้อมูลจำลองซ้ำ'],['5','สาธิต Sorting เลือกอัลกอริทึม เกณฑ์ ทิศทาง และ Trace'],['6','สาธิต Searching เลือก Sequential หรือ Binary และฟิลด์ค้นหา'],['7','Benchmark Sorting ทั้ง 4 วิธี'],['8','ล้างข้อมูลทั้งหมด'],['0','ออกจากโปรแกรม']]
